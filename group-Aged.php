...
// Получаем новости, относящиеся к группе "Aged" для текущей страницы
try {
  $stmt = $pdo->prepare("
  ...
  ");
  $stmt->bindValue(':group_name', 'Aged', PDO::PARAM_STR);
...
if (count($result) > 0) {
  foreach ($result as $row) {
      ...
      echo "<p>Категории: " . $row['category_names'] . "</p>";
      echo "<hr>";
  }
} else {
...
if ($pages > 1) {
  ...
  for ($i = 1; $i <= $pages; $i++) {
      if ($i == $page) {
          ...
          echo "<a href='group-aged.php?page=$i'>$i</a>";
      }
  }
  echo "</div>";
}
