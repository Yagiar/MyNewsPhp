...
try {
  ...
  ");
  $stmt->execute(['group_name' => 'Student']);
...
// Получаем новости, относящиеся к группе "Student" для текущей страницы
try {
  $stmt = $pdo->prepare("
  ...
  ");
  $stmt->bindValue(':group_name', 'Student', PDO::PARAM_STR);
...
if ($pages > 1) {
  ...
  for ($i = 1; $i <= $pages; $i++) {
      if ($i == $page) {
          ...
          echo "<a href='group-student.php?page=$i'>$i</a>";
      }
  }
  echo "</div>";
}
